...
class TestVideo(data.Dataset):
    ...
    def _colect_filelist(self, root):
        ...
        # Filter out ground truth directories (ending with _gt)
        dirs = [d for d in dirs if not os.path.basename(d).endswith('_gt')]
...
class VideoJump(Video):  
    def __getitem__(self, index):
        video_folder = str(self.videos[index][0].split('/')[-2])
        video_number =  next((i for i, sublist in enumerate(self.all_videos) if video_folder in sublist[0]), -1)        
        video_name = self.videos[index]
        ...
        return {'video': video,'video_name': video_name_jump }
