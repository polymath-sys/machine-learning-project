...
def parse_args():
    parser = argparse.ArgumentParser(description='Drone Guard')
...
def main():
    ...
    # choose device: CUDA only if available and requested in config.GPUS
    gpus = list(config.GPUS)
    device = torch.device('cuda' if torch.cuda.is_available() and len(gpus) > 0 else 'cpu')
    # expose device to train functions
    global DEVICE
    DEVICE = device

    if device.type == 'cuda' and len(gpus) > 0:
        model = nn.DataParallel(model, device_ids=gpus).to(device)
    else:
        # run on CPU without DataParallel
        model = model.to(device)

    # load checkpoint if requested
    if args.resume_train:
        ckpt = torch.load(args.model_file, map_location=device)
        state_dict = ckpt['state_dict'] if isinstance(ckpt, dict) and 'state_dict' in ckpt.keys() else ckpt
    ...
    if args.pseudo:
        ...
        train_loader_jump = torch.utils.data.DataLoader(
            train_dataset_jump,
            batch_size=config.TRAIN.BATCH_SIZE_PER_GPU * (len(gpus) if len(gpus) > 0 else 1),
    ...
    if args.test:
        ...
        test_loader = torch.utils.data.DataLoader(
            test_dataset,
            batch_size=BATCH_SIZE_PER_GPU_TEST * (len(gpus) if len(gpus) > 0 else 1),
            shuffle=False,
            num_workers=config.WORKERS,
            pin_memory=True
        )
    ...
    for epoch in range(last_epoch, config.TRAIN.END_EPOCH):
        ...
        if (epoch + 1) % config.SAVE_CHECKPOINT_FREQ == 0:
            ...
            torch.save(save_model.state_dict(), os.path.join(final_output_dir, 'epoch_{}.pth'.format(epoch + 1)))

        if (epoch % 10 == 0) and (args.output_folder is not None):
            save_model = model.module if hasattr(model, 'module') else model
            torch.save(save_model.state_dict(), os.path.join(args.output_folder, 'epoch_train_{}.pth'.format(epoch + 1)))
        if args.test and (epoch > 15):
            ...
            if auc > AUC_memory:
                ...
                save_model = model.module if hasattr(model, 'module') else model
                torch.save(save_model.state_dict(), os.path.join(final_output_dir, 'best_AUC.pth'.format(epoch + 1)))
                if args.output_folder is not None:
                    torch.save(save_model.state_dict(), os.path.join(args.output_folder, 'best_AUC.pth'.format(epoch + 1)))                                               
                AUC_memory = auc
    ...
    save_model = model.module if hasattr(model, 'module') else model
    torch.save(save_model.state_dict(), final_model_state_file)
...
def train(config, train_loader, model, loss_functions, optimizer, epoch, logger, args):
    ...
    for i, data in enumerate(train_loader):
        ...
        target_model = model.module if hasattr(model, 'module') else model
        output, loss_commit = target_model.compute_loss(inputs)
...
def train_pseudo(config, train_loader, train_loader_jump, model, loss_functions, optimizer, epoch, logger, pseudolossepoch, pseudolosscounter, args):
    ...
    for i, (data, data_jump) in enumerate(zip(train_loader, train_loader_jump)):
        ...
        # Batch Data
        batch_size = config.TRAIN.BATCH_SIZE_PER_GPU * (len(list(config.GPUS)) if len(list(config.GPUS)) > 0 else 1)
        inputs_batch = train_util.To_Batch(inputs, batch_size, config.MODEL.ENCODED_FRAMES)
        inputs_batch_jump = train_util.To_Batch(inputs_jump, batch_size, config.MODEL.ENCODED_FRAMES)
        ...
        for b in range(batch_size):
            ...
            pseudo_bool = False
            pseudo_anomaly_jump = total_pseudo_prob <= rand_number < total_pseudo_prob + config.DATASET.PSEUDO_ANOMALY_PROB
            total_pseudo_prob += config.DATASET.PSEUDO_ANOMALY_PROB
            ...
            cls_labels.append(0 if pseudo_bool else 1)
        cls_labels = torch.Tensor(cls_labels).unsqueeze(1).to(DEVICE)
        
        # Frame Data 
        inputs = train_util.To_Frame(inputs_batch, batch_size, config.MODEL.ENCODED_FRAMES)
        inputs = [inp.to(DEVICE) for inp in inputs]
        target_model = model.module if hasattr(model, 'module') else model
        output, loss_commit_ = target_model.compute_loss(inputs)
